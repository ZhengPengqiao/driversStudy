 ...
 * @author zpq (18-7-5)
 */
int hello_chdev_major = 250; /*< 主设备号 */
int hello_chdev_minor = 0; /*< 次设备号 */
int devno;  /*< 记录设备号 */
struct cdev hello_chdev_cdev;   /*< 字符设备对象 */
...
static int __init hello_chdev_init(void)
{
	...
	ret = register_chrdev_region(devno, 1, MYDEV_NAME);
	if(ret < 0)
	{
	...
	cdev_init(&hello_chdev_cdev,&hello_chdev_fops);
	ret = cdev_add(&hello_chdev_cdev, devno, 1);
	if(ret < 0)
	{

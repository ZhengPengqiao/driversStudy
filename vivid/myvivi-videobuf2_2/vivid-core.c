...
static int vid_cap_nr = -1;
module_param(vid_cap_nr, int, 0444);
MODULE_PARM_DESC(vid_cap_nr, " videoX start number, -1 is autodetect");

static int vid_out_nr = -1;
module_param(vid_out_nr, int, 0444);
MODULE_PARM_DESC(vid_out_nr, " videoX start number, -1 is autodetect");

static int vbi_cap_nr = -1;
module_param(vbi_cap_nr, int, 0444);
MODULE_PARM_DESC(vbi_cap_nr, " vbiX start number, -1 is autodetect");

static int vbi_out_nr = -1;
module_param(vbi_out_nr, int, 0444);
MODULE_PARM_DESC(vbi_out_nr, " vbiX start number, -1 is autodetect");

static int sdr_cap_nr = -1;
module_param(sdr_cap_nr, int, 0444);
MODULE_PARM_DESC(sdr_cap_nr, " swradioX start number, -1 is autodetect");

static int radio_rx_nr = -1;
module_param(radio_rx_nr, int, 0444);
MODULE_PARM_DESC(radio_rx_nr, " radioX start number, -1 is autodetect");

static int radio_tx_nr = -1;
module_param(radio_tx_nr, int, 0444);
MODULE_PARM_DESC(radio_tx_nr, " radioX start number, -1 is autodetect");

static int ccs_cap_mode = -1;
module_param(ccs_cap_mode, int, 0444);
MODULE_PARM_DESC(ccs_cap_mode, " capture crop/compose/scale mode:\n"
			   "\t\t    bit 0=crop, 1=compose, 2=scale,\n"
			   "\t\t    -1=user-controlled (default)");

static int ccs_out_mode = -1;
module_param(ccs_out_mode, int, 0444);
MODULE_PARM_DESC(ccs_out_mode, " output crop/compose/scale mode:\n"
			   "\t\t    bit 0=crop, 1=compose, 2=scale,\n"
			   "\t\t    -1=user-controlled (default)");

static unsigned multiplanar =  1;
module_param(multiplanar, uint, 0444);
MODULE_PARM_DESC(multiplanar, " 1 (default) creates a single planar device, 2 creates a multiplanar device.");

/* Default: video + vbi-cap (raw and sliced) + radio rx + radio tx + sdr + vbi-out + vid-out */
static unsigned node_types = 0x0001;
module_param(node_types, uint, 0444);
...
static struct vivid_dev *vivid_devs;
...
static int vivid_create_instance(struct platform_device *pdev)
{
	static const struct v4l2_dv_timings def_dv_timings =
					V4L2_DV_BT_CEA_1280X720P60;
	unsigned in_type_counter[4] = { 0, 0, 0, 0 };
	unsigned out_type_counter[4] = { 0, 0, 0, 0 };
	int ccs_cap = ccs_cap_mode;
	int ccs_out = ccs_out_mode;
	bool has_tuner;
	bool has_modulator;
	struct vivid_dev *dev;
	struct video_device *vfd;
	struct vb2_queue *q;
	unsigned node_type = node_types;
	...
	for (i = 0; i < dev->num_inputs; i++) {
		dev->input_type[i] = (input_types >> (i * 2)) & 0x3;
	...
	for (i = 0; i < dev->num_outputs; i++) {
		dev->output_type[i] = ((output_types >> i) & 1) ? HDMI : SVID;
	...
	if (dev->has_vid_cap) {
		...
		ret = video_register_device(vfd, VFL_TYPE_GRABBER, vid_cap_nr);
		if (ret < 0)
			goto unreg_dev;
	...
	if (dev->has_vid_out) {
		...
		ret = video_register_device(vfd, VFL_TYPE_GRABBER, vid_out_nr);
		if (ret < 0)
			goto unreg_dev;
	...
	if (dev->has_vbi_cap) {
		...
		ret = video_register_device(vfd, VFL_TYPE_VBI, vbi_cap_nr);
		if (ret < 0)
			goto unreg_dev;
	...
	if (dev->has_vbi_out) {
		...
		ret = video_register_device(vfd, VFL_TYPE_VBI, vbi_out_nr);
		if (ret < 0)
			goto unreg_dev;
	...
	if (dev->has_sdr_cap) {
		...
		ret = video_register_device(vfd, VFL_TYPE_SDR, sdr_cap_nr);
		if (ret < 0)
			goto unreg_dev;
		v4l2_info(&dev->v4l2_dev, "V4L2 capture device registered as %s\n",
					  video_device_node_name(vfd));
...
static int vivid_probe(struct platform_device *pdev)
{
	...
	ret = vivid_create_instance(pdev);
	if (ret) {
